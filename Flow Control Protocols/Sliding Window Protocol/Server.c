...
int main()
{
	...
	printf("*************************SELECTIVE REPEAT RECEIVER SIDE*************************\n");
    char message[150];
    ...
    int win_size[1];
    recv(client_socket, win_size, sizeof(win_size), 0);//receiving the window size from sender
    ...
    int message_len = 0;
	recv(client_socket, reached_stringend, sizeof(reached_stringend), 0); //if reached string end or not

    //sending continuous acknowledgement for received packets in a given window size
    while(reached_stringend[0] == '0')
    {
        for(int j=0; j<windowsize; j++)//receiving packets for a given window size
        {
        ...
        printf("\n");
        for(int j=0; j<windowsize; j++) //sending acknowledgements for received packets
        {
			...
			send(client_socket, time_taken, sizeof(time_taken), 0); //send the time taken to give acknowledgement for received packet

			if(acknowledge[0] == '0' || timeout < duration) //Negative acknowlegment
            {
